...
class User:
    ...
    def __init__(self):
        self.levelArray = [-8, -7, -6, -5, -4, -3, -2, -1, 1, 2, 3, 4, 5, 6, 7, 8]
        self.rank = self.levelArray[0]
        self.progess = 0
    ...
    def inc_progress(self, level):
        indexDiff = self.levelArray.index(level) - self.levelArray.index(self.rank)
        if indexDiff == -1:
            self.progess += 1
        elif indexDiff == 0:
            self.progess += 3
        elif indexDiff > 0:
            self.progess += 10*(indexDiff)**2
        self.rank = self.levelArray[self.levelArray.index(self.rank) + self.progess//100]
        self.progess = self.progess%100


Franky = User()
Franky.inc_progress(-6)
print(Franky.rank)
Franky.inc_progress(-8)
Franky.inc_progress(-5)
Franky.inc_progress(-1)
Franky.inc_progress(1)
print(Franky.progess, Franky.rank)
Franky.inc_progress(2)
print(Franky.progess, Franky.rank)
